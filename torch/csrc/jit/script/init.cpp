...
struct ParameterListValue : public SugaredValue {
  ParameterListValue(py::object self, std::shared_ptr<Module> module)
...
struct ModuleValue : public SugaredValue {
  ...
  std::shared_ptr<SugaredValue> attr(SourceRange loc, Method & m, const std::string& field) override {
    ...
    // if (py::isinstance(py::getattr(py_module, field.c_str()), py::module::import("torch.jit").attr("OrderedParameterDict"))) {
      ...
      // if (field == "values") {
      return std::make_shared<ParameterListValue>(py_module, module);
      // }
